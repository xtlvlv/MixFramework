...
public static class GizmosExtensions {
	...
	public static void DrawWireCircle2D(Vector3 center, float radius, int segments = 20, Quaternion rotation = default(Quaternion))
	{
		DrawWireArc2D(center,radius,360,segments,rotation);
	}
	...
	public static void DrawWireArc2D(Vector3 center, float radius, float angle, int segments = 20, Quaternion rotation = default(Quaternion))
	{
			
		var old = Gizmos.matrix;
		
		Gizmos.matrix = Matrix4x4.TRS(center,rotation,Vector3.one);
		Vector3 from = Vector3.up * radius;
		var step = Mathf.RoundToInt(angle / segments);
		for (int i = 0; i <= angle; i += step) {
			var to = new Vector3(radius * Mathf.Sin(i * Mathf.Deg2Rad), radius * Mathf.Cos(i * Mathf.Deg2Rad), 0);
			Gizmos.DrawLine(from, to);
			from = to;
		}

    ...
    public class ExcelMiddleData
    {
        ...
        public void Init(DataTable sheet, string excelPath)
        {
            ...
            for (int i = ExcelConvertPathSetting.EXCEL_ROW_INDEX_Content_Start; i < realRowCount; i++)
            {
                ...
                if (sheet.Rows[i][0].ToString().StartsWith("#")) continue;      // 空行 或 以#开头的行跳过。
                    ...
                    if (string.IsNullOrEmpty(propValue))
                    {
                        switch (propType)
                        {
                            ...
                            case "bool":
                                ...
                                break;
                            default:
                                throw new Exception($"异常类型,title:【{propTitle}】, row:【{i + 1}】, path:" + excelPath);
                        }
                    }
